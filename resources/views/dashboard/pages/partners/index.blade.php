            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-lg-flex">
                        <div>
                            <h6 class="text-white text-capitalize ps-3">Partners</h6>
                        </div>
                        <div class="ms-auto my-auto mt-lg-0 mt-4 me-3">
                            <div class="ms-auto my-auto">
                                <a href="{{ route('admin.partners.create') }}" class="btn bg-gradient-dark btn-sm mb-0">+
                                    New Partner</a>
                            </div>
                        </div>
                    </div>
                                ...
                                @foreach ($data as $index => $partner)
                                    <tr>
                                        ...
                                        <td class="align-middle text-center">
                                            @if ($partner->image)
                                                <img src="{{ $partner->image_full_path }}" class="avatar border-radius-lg"
                                                    width="50px" height="50px">
                                            @endif
                                        </td>
                                        <td class="align-middle text-center">
                                            <a class="btn btn-link text-dark px-3 mb-0"
                                                href="{{ route('admin.partners.edit', ['partner' => $partner->id]) }}"><i
                                                    class="material-symbols-rounded text-sm me-2">edit</i>Edit</a>
                                            <form style="display: inline" method="post"
                                                action="{{ route('admin.partners.destroy', ['partner' => $partner->id]) }}">
                                                @csrf
                                                @method('DELETE')
